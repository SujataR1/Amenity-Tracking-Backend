    ...
    try:
        async with in_transaction():
            for month, consumption in enumerate(monthly_consumptions, start=1):
                ...
                existing_record = await GasConsumption.get_or_none(
                    user_id=user_id, year=year, month=month_name
                )
                if not existing_record:
                    await GasConsumption.create(
                        user_id=user_id,
                        year=year,
                        month=month_name,
                    ...
                    print(
                        f"Saved consumption for {user_email} {year}-{month_name}: {consumption} kg"
                    )
                else:
                    print(
...
async def process_user(user):
    ...
    answers = {
        "one": questionnaire.one,  # Number of people
        "twelve": questionnaire.twelve,  # Daily heavy cooking
    }
