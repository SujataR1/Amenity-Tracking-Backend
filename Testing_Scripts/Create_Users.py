...
# Zip codes
ZIP_CODES = [f"{10000 + i}" for i in range(10)]  # Generate 10 zip codes (10000 to 10009)
...
def generate_phone_number():
    return random.randint(1000000000, 9999999999)  # Generate 10-digit phone numbers
    ...
    try:
        async with in_transaction():
            for zip_code in ZIP_CODES:  # Loop through each zip code
                for _ in range(50):  # 50 users per zip code
                    user_data, raw_password = generate_user_data(user_count, zip_code)
                    try:
                        ...
                        print(f"User {user_count} created successfully: {user_data['email']} in ZIP {zip_code}")
                        write_to_json(user_data["email"], raw_password)  # Store raw password in JSON
